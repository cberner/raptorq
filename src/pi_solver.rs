        ...
        for op in self.deferred_D_ops.iter() {
            match op {
                SymbolOps::AddAssign { dest, src } => {
                    debug_assert!(*dest < d_len && *src < d_len && dest != src);
                    // SAFETY: dest != src is guaranteed by the algorithm (a symbol is never
                    // added to itself), and both indices are within bounds.
                    unsafe {
                ...
                SymbolOps::FMA { dest, src, scalar } => {
                    debug_assert!(*dest < d_len && *src < d_len && dest != src);
                    // SAFETY: same as AddAssign case above
                    unsafe {
                        let dest_sym = &mut *d_ptr.add(*dest);
                        let src_sym = &*d_ptr.add(*src);
