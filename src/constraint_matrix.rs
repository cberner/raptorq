...
pub fn generate_constraint_matrix<T:Iterator<Item=u32>>(source_block_symbols: u32, encoded_symbol_indices: T) -> OctetMatrix {
    ...
    // I_S
    for i in 0..S {
        matrix.set::<Octet>(i as usize, i + B as usize, 1.into());
    }

    // G_LDPC,2
    // See section 5.3.3.3
    for i in 0..S {
        matrix.set::<Octet>(i, (i % P) + W, 1.into());
        matrix.set::<Octet>(i, ((i + 1) % P) + W, 1.into());
    }
    ...
    // I_H
    for i in 0..H {
        matrix.set::<Octet>(i + S as usize, i + (Kprime + S) as usize, 1.into());
    }
